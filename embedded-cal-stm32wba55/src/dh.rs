...
// Total PKA RAM size (RM0493 / ST HAL `stm32wbaxx_hal_pka.c`, `PKA->RAM[1334]`), byte offset
// 0x400..0x18D4. Must cover every slot index used below (e.g. RAM_K=936, RAM_SIGN_PRIVATE_D=714)
// so that `pka_zero_ram` actually clears secret material -- a prior value of 667 left everything
// past that index (including the DH scalar at RAM_K) never zeroed despite the call sites' intent.
const PKA_RAM_WORDS: usize = 1334;

// PKA RAM slot indices for ECDSA-P256 signature generation (PKA_MODE_ECDSA_SIGN = 0x24).
// Shares its curve-parameter/scalar/base-point slots (RAM_N_LEN, RAM_P_LEN, RAM_A_SIGN, RAM_A,
// RAM_B, RAM_P, RAM_N, RAM_K, RAM_POINT_X, RAM_POINT_Y) with ECC_MULT above -- confirmed identical
// byte offsets in ST's `stm32wba55xx.h` (`PKA_ECDSA_SIGN_IN_*` vs `PKA_ECC_MUL_IN_*`).
...
// Unlike ECDSA_SIGN, this mode uses its own distinct layout (confirmed against ST's
// `PKA_ECDSA_VERIF_IN_*`/`_OUT_*` offsets in `stm32wba55xx.h`) -- none of the ECC_MULT/ECDSA_SIGN
// slot constants above apply here.
