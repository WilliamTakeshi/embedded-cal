...
/// Tests that exercise the [EC plumbing][embedded_cal::plumbing::ec] directly, without going
/// through a [`DhProvider`][embedded_cal::DhProvider].
///
/// These are separate from [`EccVector::test_with()`] because [`EcPrimitives`] is parametrized
/// over a type level curve, whereas [`EccVector`] carries its curve as a run-time COSE number --
/// so there is one method per curve rather than one dispatching method.
///
/// Each of them does nothing if the back-end does not advertise
/// [`HAS_MULTIPLY_SCALAR_POINT`][EcPrimitives::HAS_MULTIPLY_SCALAR_POINT], which is how a back-end
/// that parks a curve on [`EmptyCal`][embedded_cal::empty::EmptyCal] passes them.
impl EccVector {
    /// Runs the vector through [`EcPrimitives<P256>`].
    ///
    /// Unlike the [`DhProvider`][embedded_cal::DhProvider] level, this does not exercise the
    /// compact (x-only) public key format: the peer's y coordinate is recovered up front through
    /// [`embedded_cal::p256::p256_recover_y()`], and the base point is passed in with both its
    /// coordinates known.
    pub fn test_plumbing_p256<E: Ec>(&self, ec: &mut E) {
        assert_eq!(self.ecdh_curve, 1, "vector is not a P-256 vector");
        if !<E::PrimitivesP256 as EcPrimitives<P256>>::HAS_MULTIPLY_SCALAR_POINT {
            return;
        }
        let p256 = ec.p256();
        ...
        for (private, public) in [
            ...
            // Scalar import/export round trip, independent of any multiplication: this is what
            // catches a byte order error in the back-end's scalar conversion.
            let d = p256
    ...
    /// Runs the vector through [`EcPrimitives<X25519>`].
    ///
    /// The scalars are clamped before import, as [`EcPrimitives`] does not do that itself.
    pub fn test_plumbing_x25519<E: Ec>(&self, ec: &mut E) {
        assert_eq!(self.ecdh_curve, 4, "vector is not an X25519 vector");
        if !<E::PrimitivesX25519 as EcPrimitives<X25519>>::HAS_MULTIPLY_SCALAR_POINT {
            return;
        }
        ...
        for (private, public) in [
            ...
            embedded_cal::rfc7748::clamp_x25519(&mut scalar);
            let d = x25519
        ...
        for (private, peer) in [
            ...
            embedded_cal::rfc7748::mask_u_x25519(&mut peer_u);
    ...
    /// Runs the vector through [`EcPrimitives<X448>`].
    ///
    /// The scalars are clamped before import, as [`EcPrimitives`] does not do that itself.
    pub fn test_plumbing_x448<E: Ec>(&self, ec: &mut E) {
        assert_eq!(self.ecdh_curve, 5, "vector is not an X448 vector");
        if !<E::PrimitivesX448 as EcPrimitives<X448>>::HAS_MULTIPLY_SCALAR_POINT {
            return;
        }
        let x448 = ec.x448();
        ...
        for (private, public) in [
            ...
            embedded_cal::rfc7748::clamp_x448(&mut scalar);
            let d = x448
                .import_scalar_bytes(&scalar)
                .expect("test vector scalar rejected");
            assert_eq!(
...
fn montgomery_point<C: Curve, P: EcPrimitives<C>>(primitives: &mut P, u: &[u8]) -> P::Point {
    // Sized off `u`, which is already in the curve's native scalar length.
