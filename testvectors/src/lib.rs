...
pub struct HkdfVector {
    pub ikm: &'static [u8],
    /// `None` triggers the all-zeros default salt (RFC 5869).
...
pub fn test_hkdf_sha256<Cal: embedded_cal::HkdfProvider>(cal: &mut Cal) {
    ...
    let hkdf_sha256 = Cal::Algorithm::from_cose_number(-10i8)
        .expect("HkdfProvider must recognize COSE -10 (HKDF-SHA-256)");
