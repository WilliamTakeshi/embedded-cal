...
// P-256 curve constants and bignum helpers shared by hardware DH back-ends. Modular
// arithmetic itself (add/mul/pow mod p) is done in hardware by each back-end (CRACEN on
// nRF54L15, PKA on STM32WBA55); this module no longer contains a software implementation.
//
...
// P-256 coefficient a = -3 mod p, as a full field element (used by hardware back-ends that
// compose their own modular arithmetic to recover y from x, e.g. embedded-cal-stm32wba55's
// pka_recover_y). Computed at compile time via `sub256`, which has no other caller.
