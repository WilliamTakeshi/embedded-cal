...
impl<EC: ExtenderConfig> HkdfProvider for Extender<EC> {
    ...
    fn hkdf_extract(
        ...
        let prk = self.hmac_with_keydata(HmacAlgorithm::HmacSha256, salt, ikm);
    ...
    fn hkdf_expand(
        ...
        while offset < okm.len() {
            let mut hmac_state = self.init_with_keydata(HmacAlgorithm::HmacSha256, &state.prk);
            self.update(&mut hmac_state, &t_prev[..t_prev_len]);
            self.update(&mut hmac_state, info);
            self.update(&mut hmac_state, &[counter]);
            let t = self.finalize(hmac_state);
