...
// Opcodes for full ECDSA-P256 sign/verify (PK_OP_ECDSA_GEN = 0x30, PK_OP_ECDSA_VER = 0x31,
// sdk-nrf regs_commands.h). Unlike PKE_OPCODE_ECC_MULT, these compute the entire signature
// generation/verification -- including the modular inverse of the nonce -- inside the PKE
// engine, so no software big-number math (`embedded_cal::p256`'s `*_mod_n` functions) is needed.
//
// Their operand slots are fixed by the microcode (unlike ECC_PTMUL's, which are configurable via
// the POINTERS register): sdk-nrf's `sx_pk_select_ops()` only reprograms POINTERS when the
// command definition's `selected_ptrA` is nonzero, and it is 0 for both these opcodes
// (pkhardware_ba414e.c `CMD_ECDSA_GEN`/`CMD_ECDSA_VER`). So, unlike `cracen_p256_mult`, no
// `pointers()` write is needed here.
